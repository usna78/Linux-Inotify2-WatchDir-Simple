...
sub validate {
    ...
    $self->logger->info("Name: " . $config->data->{name}) if $config->data->{name};
...
sub run {
    ...
    # Main event loop - simple polling with 1 second interval
    while ($self->running) {
        # Check for signals
        ...
        # Poll for inotify events with 1 second timeout
        $self->inotify->poll() or do {
            # Timeout - just continue loop to check signals
            sleep 1;
        };
    }
